			...
			if distance <= 50.0 then
				if IsEntityAVehicle(vehicle) and GetVehicleClass(vehicle) ~= 9 and GetPedInVehicleSeat(vehicle,-1) == ped then
					...
					if distance <= 10.9 then
						drawTxt("PRESSIONE  ~r~E~w~  PARA INICIAR A CORRIDA",6,0.09,0.770,0.50,255,255,255,150)
						if IsControlJustPressed(0,38) and emP.ticketCheck() then
							 ...
							 if explosive >= 40 then
							 	...
							 	TriggerEvent("Notify","negado","Você começou uma corrida <b>Explosiva</b>, não saia do veículo e termine<br>no tempo estimado, ou então seu veículo vai explodir com você dentro, boa sorte.")
							end
						end
					end
				end
