...
emP = Tunnel.getInterface("vrp_liberty")
...
local garages = {
	[1] = { locked = false, camera = { x = 120.28, y = -1994.89, z= 17.62, heading = 72.581535339355 }, driveout = { x = 120.28, y = -1994.89, z= 17.62, heading = 72.581535339355 }, drivein = { x = 120.28, y = -1994.89, z= 17.62, heading = 72.581535339355 },  outside = { x = 120.28, y = -1994.89, z= 17.62, heading = 72.581535339355 }, inside = { x = 120.28, y = -1994.89, z= 17.62, heading = 72.581535339355 }}
...
local LSCMenu = Menu.new("Mecânica","CATEGORIES",0.16,0.13,0.24,0.36,0,{255,255,255,255})
...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		scol = respray:addSubMenu("PEARLY COLORS","Pearly colors",nil,true)
										...
										if not LSC_Config.lock then
											if IsControlJustPressed(1,201) then
												if emP.checkPermission() then
													...
													TriggerEvent("Notify","negado","Apenas Membros da <b>liberty</b> podem modificar veículos.",8000)
												end
											else
												drawTxt("",4,0.5,0.93,0.50,255,255,255,180)
											end
										else
											drawTxt("~y~Liberty~w~ OCUPADA NO MOMENTO",4,0.5,0.93,0.50,255,255,255,180)
										end
									else
										if IsControlJustPressed(1,201) then
											if emP.checkPermission() then
								...
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									if pos.locked then
										if not LSC_Config.lock then
											...
											drawTxt("~y~LIIBERTY~w~ OCUPADA NO MOMENTO",4,0.5,0.93,0.50,255,255,255,180)
										end
									else
										inside = true
										currentpos = pos
