...
$('.arrow-right').on('click', function(e){
  e.preventDefault();
  let value = parseFloat($(this).prev().val()),
...
$('.arrow-left').on('click', function(e){
  e.preventDefault();
  let value = parseFloat($(this).next().val()),
      newValue = parseFloat(value - 1),
      max = $(this).parent().prev().attr('data-legend');
...
$('.arrowvetement-right').on('click', function(e){
  let li = $(this).parent().find('li.active'),
      active = li.next(),
      id = active.attr('data'),
...
$('.vetements .group').each(function(){
  let max = $(this).find('li:last-of-type').attr('data');
